...
questions_raw = guides_with_prompts.select(
    ...
    expr('AI_QUERY("databricks-claude-sonnet-4-6", question_prompt)').alias("questions_json")
)
...
facts_raw = questions_with_facts_prompt.select(
    "question",
    expr('AI_QUERY("databricks-claude-sonnet-4-6", fact_prompt)').alias("expected_facts_json")
)
